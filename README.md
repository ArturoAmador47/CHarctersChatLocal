...
- **Image generation intents** — characters can emit an `[IMAGE: description]` marker in their reply, which triggers on-the-fly image generation via [ComfyUI](https://github.com/comfyanonymous/ComfyUI) (`http://localhost:8188`), shown inline in the chat with live progress.
...
- (Optional) [ComfyUI](https://github.com/comfyanonymous/ComfyUI) running locally (default `http://localhost:8188`) if you want in-chat image generation
...
src/
  lib/
    api/        # OpenRouter + ComfyUI API clients
    ...
    stores/     # Svelte 5 runes-based state (characters, chat, models, ComfyUI status)
    utils/      # Image-intent parsing, etc.
...
- ComfyUI is expected to run unauthenticated on localhost — no API key required.
